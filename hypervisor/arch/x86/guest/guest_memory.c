...
/**
 * Walk a page table from user space and apply cb
 * to every leaf page
 */
void ept_copy_from_pgtable(struct acrn_vm *vm_src, struct acrn_vm *vm_dst,
	uint64_t *pml4e_dst, uint64_t *pml4e_gpt_hva, ept_pg_copy_handler cb)
{
	uint64_t i, j, k, m;
	uint64_t *pml4e, *pdpte, *pde, *pte;

	const struct memory_ops *mem_ops = &ppt_mem_ops;

	for (i = 0UL; i < PTRS_PER_PML4E; i++) {
		pml4e = guest_pml4e_offset(pml4e_gpt_hva, i << PML4E_SHIFT);
		if (mem_ops->pgentry_present(*pml4e) == 0UL) {
			continue;
		}
		for (j = 0UL; j < PTRS_PER_PDPTE; j++) {
			pdpte = guest_pdpte_offset(vm_src, pml4e, j << PDPTE_SHIFT);
			if (mem_ops->pgentry_present(*pdpte) == 0UL) {
				continue;
			}
			if (pdpte_large(*pdpte) != 0UL) {
				cb(vm_src, vm_dst, pml4e_dst, pdpte, PDPTE_PFN_MASK, PDPTE_SIZE);
				continue;
			}
			for (k = 0UL; k < PTRS_PER_PDE; k++) {
				pde = guest_pde_offset(vm_src, pdpte, k << PDE_SHIFT);
				if (mem_ops->pgentry_present(*pde) == 0UL) {
					continue;
				}
				if (pde_large(*pde) != 0UL) {
					cb(vm_src, vm_dst, pml4e_dst, pde, PDE_PFN_MASK, PDE_SIZE);
					continue;
				}
				for (m = 0UL; m < PTRS_PER_PTE; m++) {
					pte = guest_pte_offset(vm_src, pde, m << PTE_SHIFT);
					if (mem_ops->pgentry_present(*pte) != 0UL) {
						/* well we should use PTE_PFN_MASK but anyway they are same */
						cb(vm_src, vm_dst, pml4e_dst, pte, PDE_PFN_MASK, PTE_SIZE);
					}
				}
			}
		}
